...
from mne.minimum_norm.inverse import _check_reference, _log_exp_var
...
def _gamma_map_opt(
    ...
    for itno in range(maxit):
        ...
        CM = np.dot(G * gammas[np.newaxis, :], G.T) # NOTE: Compute the Covariance Matrix (CM)
        
        # NOTE: Modify the Diagonal Elements -> add the noise variance σ^2
        CM.flat[:: n_sensors + 1] += alpha 
        
        # NOTE: If we want to compute the pseudo-inverse of the matrix CM, we can use the results of the SVD: 
        # CM^{-1} = V S^{-1} U^T. 
        # This pseudo-inversion is useful when CM is not invertible
        ...
        # NOTE: Compute the pseudo-inverse of CM using the formula:
        # CM^{-1} = U S^{-1} U^T
        CMinv = np.dot(U / (S + eps), U.T)
        CMinvG = np.dot(CMinv, G)
        
        # NOTE: A = ((L*γ * L^T + α)^-1) L)^T * y
        # NOTE: in latex:
        # A = \left( \left( L \cdot \gamma \cdot L^T + \alpha \right)^{-1} L \right)^T \cdot y
    ...
    x_active = n_const * gammas[:, None] * A # (88, 300)
    
    posterior_cov = gammas - gammas * G_CMinvG * gammas[:, np.newaxis] # (88, 88) # NOTE: todo check this again.
...
def gamma_map(
    ...
    X, active_set, posterior_cov = _gamma_map_opt(
        M,
        gain,
        alpha,
        maxit=maxit,
    ...
    if return_residual:
        return out, residual, posterior_cov
        
    return out, posterior_cov
